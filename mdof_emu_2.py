...
class MDOFDataCache:
    ...
    def _get_cache_key(self, n_samples, n_dof, segment_time, seed):
        """生成唯一的缓存键"""
        params_str = f"{n_samples}_{n_dof}_{segment_time}_{seed}"
        return hashlib.md5(params_str.encode()).hexdigest()
    
    def load_or_generate(self, n_samples, n_dof=10, segment_time=10.0, 
                         force_regen=False, generator_func=None, seed=42):
        """加载缓存或生成新数据"""
        cache_key = self._get_cache_key(n_samples, n_dof, segment_time, seed)
        ...
        if generator_func:
            signals, images, labels = generator_func(n_samples, n_dof, segment_time)
        else:
...
def generate_mdof_dataset_parallel(n_samples=100, n_dof=10, 
    ...
    def generator_func(n_samples, n_dof, segment_time):
        """内部生成函数"""
        # 创建任务
        np.random.seed(seed)
        tasks = []
        ...
        results = Parallel(n_jobs=n_jobs, backend='threading', verbose=0)(
            delayed(generate_single_sample)(s, l, p)
            for s, l, p in tqdm(tasks, desc="并行生成", leave=False)
        )
...
if __name__ == "__main__":
    print("=" * 70)
    print("优化后的MDOF仿真系统")
    print("Integrated: Numba JIT, Streaming, Parallel, Caching")
    print("=" * 70)
    ...
    signals, images, labels = generate_mdof_dataset_parallel(
        ...
        segment_time=10.0,
        n_jobs=-1,
        force_regen=False,  # 设为 True 强制重新生成
        seed=42
    )
    end_time = time.time()
