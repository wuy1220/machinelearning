...
class ImprovedJacketPlatformSimulator:
    ...
    def generate_excitation(self, 
        ...
        if excitation_type == 'filtered_noise':
            raw_noise = np.random.randn(num_steps)
            ...
            F[:, target_dof] = signal.sosfiltfilt(sos, raw_noise) * amplitude
            ...
            F[:, target_dof] = amplitude * np.sin(phase)
            
        elif excitation_type == 'random':
            F[:, target_dof] = np.random.randn(num_steps, n) * amplitude * 0.1
...
class TimeStackedGVRFeatureExtractor:
    ...
    def extract_stacked_gvr_features(self, damaged_signal, healthy_signal):
        ...
        step_size_for_image = self.num_stack_windows // 3  # 重叠采样，增加样本数
    ...
    def generate_intra_window_feature_maps(self, damaged_signal, healthy_signal, 
                                          image_size=(224, 224), 
                                          intra_window_segments=100):
        """
        生成包含窗口内微观时序演化的特征图
        
        Args:
            damaged_signal: 损伤信号 (total_steps, num_channels)
            healthy_signal: 健康信号 (total_steps, num_channels)
            image_size: 输出图像尺寸
            intra_window_segments: 将每个滑动窗口内部切分的段数（决定Y轴的原始分辨率，后续会插值到224）
                                  建议设为 50-100，太小计算不稳定，太大计算慢。
        
        Returns:
            feature_maps: (num_windows, 224, 224, 3)
        """
        ...
        print(f"正在生成微观时序特征图: {num_windows} 个窗口, 每个窗口内分为 {intra_window_segments} 段...")
        ...
        for i in range(num_windows):
            ...
            # 1. 将窗口内的长信号 reshape 成段
            # 注意：需要确保能整除，或者丢弃余数，这里使用丢弃余数
            actual_len = win_d.shape[0]
            segment_len = actual_len // intra_window_segments
            if segment_len < 1:
                # 如果窗口太短，分段数少于1，则填充处理（极罕见情况）
                segment_len = 1
            ...
            # 2. 向量化计算每一段的 DI
            # DI = sqrt(sum(damaged^2)) / sqrt(sum(healthy^2)) 
            # 或者 RMS 比值，这里沿用原代码的 L2 范数形式
            
            # 分子：差异的平方和的开根号
            diff_power = np.sum((segs_d - segs_h)**2, axis=1)  # shape: (intra_window_segments, num_channels)
            ...
            # Y轴插值 (微观时间维度: intra_window_segments -> 224)
            # 利用 2D 插值或对每列插值
...
class ImprovedDamageDataGenerator:
    ...
    def _compute_single_scenario_data(self, damaged_dofs, severity_ratios, scenario_id):
        ...
        current_seed_damaged = 42000 + scenario_id * 2
        ...
        # ---------------------------------------------------------
        # 3. 提取特征 (修改部分：使用微观时序逻辑)
        # ---------------------------------------------------------
        
        # 直接传入原始信号，方法内部处理滑动窗口和微观分段
        # intra_window_segments=100 表示每个大窗口内部切100小段来计算Y轴演化
...
if __name__ == "__main__":
    ...
    generator = ImprovedDamageDataGenerator(
        ...
        output_dir='./jacket_damage_data_timespace4'
    )
