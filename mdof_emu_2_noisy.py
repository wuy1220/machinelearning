...
class OptimizedMDOFSimulator:
    ...
    def generate_feature_image(self, damaged_dofs=None, 
        ...
        主要改进:
        1. 损伤不再使用明显的红色圆圈，而是通过颜色/纹理的细微变化表示
        2. 添加高斯噪声增加难度
        3. 可选添加纹理噪声
        4. 可调节的损伤隐蔽程度
        
        参数:
            damaged_dofs: 发生损伤的自由度列表
            noise_level: 高斯噪声标准差 (0-1)，建议 0.03-0.08
            damage_subtlety: 损伤隐蔽程度 (0-1)，值越大越隐蔽，建议 0.6-0.8
        ...
        img = Image.new('RGB', (img_size, img_size), (bg_base, bg_base, bg_base+5))
        ...
        if damaged_dofs:
            for dof in damaged_dofs:
                if dof < len(points):
                    ...
                    # 根据隐蔽程度调整损伤可见性
                    # damage_subtlety 越大，损伤越隐蔽
                    visibility_factor = max(0.1, 1.0 - damage_subtlety)
                    
                    # === 改进1: 非常淡的色偏（多层渐变圆圈）===
                    max_radius = int(25 * visibility_factor)
                    if max_radius > 3:
                        for r in range(max_radius, 3, -3):
                            ...
                            base_color = bg_base
                            # 使用非常淡的红色偏移
                            color = (min(255, base_color + intensity), 
                                    base_color, 
                                    base_color)
                    ...
                    # === 改进3: 连接线细微变化 ===
                    line_width = max(1, 3 - int(1 * visibility_factor))
                    line_color_shift = int(15 * visibility_factor)
                    line_color = (60 + line_color_shift, 60, 60)
                    
                    # 左侧连接线
                    if dof > 0:
                        p_prev = points[dof-1]
                        draw.line([p_prev, (cx, cy)], line_color, width=line_width)
                    
                    # 右侧连接线
                    if dof < len(points) - 1:
                        p_next = points[dof+1]
                        draw.line([(cx, cy), p_next], line_color, width=line_width)
        ...
        if add_texture:
            # 生成轻微的纹理噪声（模拟自然纹理）
            texture = np.random.uniform(-noise_level*0.5, noise_level*0.5, img_array.shape)
            img_array = np.clip(img_array + texture, 0, 1)
